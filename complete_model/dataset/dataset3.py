...
class NucCorrDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if True:
            ...
            if self.transform and random.random() < 1:  # 50% chance to apply shift
    ...
    def _apply_seg_augmentations(self, seg):
        """Apply segmentation augmentations"""
        ...
        if self.rng.random() < 0.9:  # 75% chance to apply augmentation
    ...
    def _apply_img_augmentations(self, img, seg, dropped_regions, drop_type, dropped_blocks):
        ...
        # 50% chance to apply image drop
        if self.rng.random() < 1:  
            d, h, w = dropped_regions.shape
            
            # Apply the same type of drop as segmentation
            if drop_type == 0:  # Patch drop
                # Randomly select 75% of the dropped blocks for image drop
                num_blocks = len(dropped_blocks)
                if num_blocks > 0:
                    num_drop = max(1, int(num_blocks * 0.9))  # At least drop 1 block
