...
def split_error_correction(sample, id1_related_ids):
    """
    使用椭球拟合和互相包含判断来修正 split error。
    如果一个 ID 的椭球包含另一个 ID 的 mask（或反过来），就合并为前者。
    """
    sample = sample.copy()
    smp_size = sample.shape
    all_ids = np.unique(sample)
    all_ids = all_ids[all_ids != 0]

    final_seg = np.zeros_like(sample)
    used_ids = set()

    for main_id in id1_related_ids:
        if main_id in used_ids:
            continue

        mask_main = (sample == main_id)
        dilated_mask_main = ndimage.binary_dilation(mask_main, iterations=1)
        sample_main = np.zeros_like(sample)
        sample_main[dilated_mask_main] = main_id

        boundary_main = get_boundary(sample_main)
        resample_points_main = sample_points(boundary_main, n=10000, all=True)[0]

        if len(resample_points_main) < 100:
            final_seg[mask_main] = main_id
            used_ids.add(main_id)
            continue

        center_main, evecs_main, radii_main, _, mae_main = ellipsoid_fit(resample_points_main)
        if mae_main > 0.5:
            final_seg[mask_main] = main_id
            used_ids.add(main_id)
            continue

        corner1 = center_main - radii_main
        corner2 = center_main + radii_main
        ...
        center_local_main = center_main - min_region

        sub_volume = sample[min_region[0]:max_region[0],
                            min_region[1]:max_region[1],
                            min_region[2]:max_region[2]]
        ids_in_sub = np.unique(sub_volume)
        ids_in_sub = ids_in_sub[ids_in_sub != 0]

        for other_id in ids_in_sub:
            if other_id == main_id or other_id in used_ids:
                continue

            mask_other = (sample == other_id)
            dilated_mask_other = ndimage.binary_dilation(mask_other, iterations=1)
            sample_other = np.zeros_like(sample)
            sample_other[dilated_mask_other] = other_id

            boundary_other = get_boundary(sample_other)
            resample_points_other = sample_points(boundary_other, n=10000, all=True)[0]

            if len(resample_points_other) < 100:
                continue

            center_other, evecs_other, radii_other, _, mae_other = ellipsoid_fit(resample_points_other)
            if mae_other > 0.5:
                continue

            # Step 4: 判断是否互相包含
            center_local_other = center_other - min_region

            cond1 = inside_ellipsoid(sub_volume, other_id, center_local_main, radii_main, evecs_main)
            cond2 = inside_ellipsoid(sub_volume, main_id, center_other - min_region, radii_other, evecs_other)

            if cond1 or cond2:
                final_seg[sample == other_id] = main_id
                used_ids.add(other_id)

        # 添加主 ID 自身
        final_seg[sample == main_id] = main_id
        used_ids.add(main_id)
