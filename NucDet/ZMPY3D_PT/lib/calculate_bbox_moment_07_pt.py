...
def calculate_bbox_moment_07_pt(voxel3d, max_order, x_sample, y_sample, z_sample):
    p, q, r = torch.meshgrid(
        torch.arange(1, max_order + 2, dtype=torch.float64, device=x_sample.device),
        torch.arange(1, max_order + 2, dtype=torch.float64, device=x_sample.device),
        torch.arange(1, max_order + 2, dtype=torch.float64, device=x_sample.device),
        indexing='ij'
    )
    ...
    x_power = torch.pow(x_sample, torch.arange(1, max_order + 2, dtype=torch.float64, device=x_sample.device))

    y_sample = y_sample[1:, None]
    y_power = torch.pow(y_sample, torch.arange(1, max_order + 2, dtype=torch.float64, device=x_sample.device))

    z_sample = z_sample[1:, None]
    z_power = torch.pow(z_sample, torch.arange(1, max_order + 2, dtype=torch.float64, device=x_sample.device))
